...
$template_parent = isset($_GET['fid']) ? $_GET['fid'] : 0;

$template = $conn->query("SELECT * FROM template where parent_id = $template_parent and user_id = '".$_SESSION['login_id']."'  order by name asc");

// Fetch the number of files for each folder
$folderCounts = array();
$filesCountQuery = $conn->query("SELECT folder_id, COUNT(*) as count FROM template_files where user_id = '".$_SESSION['login_id']."' GROUP BY folder_id");
...
<div class="container-fluid">
	<div class="col-lg-12">
		<div class="pagetitle">
			...
			while ($id > 0) {
				$path = $conn->query("SELECT * FROM template where id = $id  order by name asc")->fetch_array();
				array_unshift($breadcrumbs, '<li class="breadcrumb-item"><a href="index.php?page=template&fid=' . $path['id'] . '">' . $path['name'] . '</a></li>');
				$id = $path['parent_id'];
			}
			...
			if (!empty($breadcrumbs)) {
				...
				echo '<nav><ol class="breadcrumb"><li class="breadcrumb-item"><a href="index.php?page=template"> Template </a></li>' . implode('', $breadcrumbs) . '</ol></nav>';
			} else {
		...
		<h5><b>Folders</b></h5>
			...
			<?php
				...
				while ($row = $template->fetch_assoc()):
					...
					$subfolderCountQuery = $conn->query("SELECT COUNT(*) as count FROM template WHERE parent_id = $folderId");
								...
								<div class="card-body">
									<table width="100%">
										...
										<?php 
											...
											while ($row = $template_files->fetch_assoc()):
												do {
												...
												<tr class='file-item' data-id="<?php echo $row['id'] ?>" data-name="<?php echo $name ?>" style="background-color: <?php echo $random_color; ?>">
													<td>
															<a href="display_file_template.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-outline-primary" title="View">
																<i class="fa fa-eye"></i>
															</a>
														</td>
...
<script>
	...
	$('#new_folder').click(function(){
		uni_modal('','manage_folder_template.php?fid=<?php echo $template_parent ?>')
	})
	$('#new_file').click(function(){
		uni_modal('','manage_files_template.php?fid=<?php echo $template_parent ?>')
	})
	$('.folder-item').dblclick(function(){
		location.href = 'index.php?page=template&fid='+$(this).attr('data-id')
	})
	...
	$("div.custom-menu .edit").click(function(e){
		e.preventDefault()
		uni_modal('Rename Folder','manage_folder_template.php?fid=<?php echo $template_parent ?>&id='+$(this).attr('data-id') )
	})
	$("div.custom-menu .delete").click(function(e){
		e.preventDefault()
		_conf("Are you sure to delete this Folder?",'delete_folder_template',[$(this).attr('data-id')])
	})
})
	...
	$("div.file.custom-menu .delete").click(function(e){
		e.preventDefault()
		_conf("Are you sure to delete this file?",'delete_file_template',[$(this).attr('data-id')])
	})
	$("div.file.custom-menu .download").click(function(e){
		e.preventDefault()
		window.open('download_template.php?id='+$(this).attr('data-id'))
	})
	...
	$('.rename_file').keypress(function(e){
		...
		if(e.which == 13){
			start_load()
			$.ajax({
				url:'ajax.php?action=file_rename_template',
				method:'POST',
				data:{id:$(this).attr('data-id'),name:$(this).val(),type:$(this).attr('data-type'),folder_id:'<?php echo $template_parent ?>'},
...
//FILE
	...
	function delete_folder_template($id){
		start_load();
		$.ajax({
			url:'ajax.php?action=delete_folder_template',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp == 1){
	...
	function delete_file_template($id){
		start_load();
		$.ajax({
			url:'ajax.php?action=delete_file_template',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp == 1){
