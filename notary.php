...
$notary_parent = isset($_GET['fid']) ? $_GET['fid'] : 0;

$notary = $conn->query("SELECT * FROM notary where parent_id = $notary_parent and user_id = '".$_SESSION['login_id']."'  order by name asc");

// Fetch the number of files for each folder
$folderCounts = array();
$filesCountQuery = $conn->query("SELECT folder_id, COUNT(*) as count FROM notary_files where user_id = '".$_SESSION['login_id']."' GROUP BY folder_id");
while ($row = $filesCountQuery->fetch_assoc()) {
    $folderCounts[$row['folder_id']] = $row['count'];
}

$notary_files = $conn->query("SELECT * FROM notary_files where folder_id = $notary_parent and user_id = '".$_SESSION['login_id']."'  order by name asc");
?>
...
<div class="container-fluid">
	<div class="col-lg-12">
		<div class="pagetitle">
			...
			while ($id > 0) {
				$path = $conn->query("SELECT * FROM notary where id = $id  order by name asc")->fetch_array();
				array_unshift($breadcrumbs, '<li class="breadcrumb-item"><a href="index.php?page=notary&fid=' . $path['id'] . '">' . $path['name'] . '</a></li>');
				$id = $path['parent_id'];
			}
			...
			if (!empty($breadcrumbs)) {
				...
				echo '<nav><ol class="breadcrumb"><li class="breadcrumb-item"><a href="index.php?page=notary"> Notary </a></li>' . implode('', $breadcrumbs) . '</ol></nav>';
			} else {
		...
		<h5><b>Folders</b></h5>
			...
			<?php
				...
				while ($row = $notary->fetch_assoc()):
					...
					$subfolderCountQuery = $conn->query("SELECT COUNT(*) as count FROM notary WHERE parent_id = $folderId");
					$subfolderCount = $subfolderCountQuery->fetch_assoc()['count'];

					if ($folderCount % 4 == 0):
						if ($folderCount != 0): // Close the previous row if it's not the first row
							echo '</div>';
						endif;
						echo '<div class="row">';
					endif;
					...
					<div class="card info-card revenue-card">
						<div class="card-body">
							<div class="d-flex align-items-center">
								...
								<img class="logo" src="assets/img/notary.png" width="60" height="60" alt="Folder Icon">
								</div>
								<div class="ps-3">
									<h1 class="card-title"><?php echo $folderName ?></h1>
								...
								<div class="card-body">
									<table width="100%">
										...
										<?php 
											...
											while ($row = $notary_files->fetch_assoc()):
												do {
												...
												<tr class='file-item' data-id="<?php echo $row['id'] ?>" data-name="<?php echo $name ?>" style="background-color: <?php echo $random_color; ?>">
													<td>
															<a href="display_file_notary.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-outline-primary" title="View">
																<i class="fa fa-eye"></i>
															</a>
														</td>
...
<script>
	...
	$('#new_folder').click(function(){
		uni_modal('','manage_folder_notary.php?fid=<?php echo $notary_parent ?>')
	})
	$('#new_file').click(function(){
		uni_modal('','manage_files_notary.php?fid=<?php echo $notary_parent ?>')
	})
	$('.folder-item').dblclick(function(){
		location.href = 'index.php?page=notary&fid='+$(this).attr('data-id')
	})
	...
	$("div.custom-menu .edit").click(function(e){
		e.preventDefault()
		uni_modal('Rename Folder','manage_folder_notary.php?fid=<?php echo $notary_parent ?>&id='+$(this).attr('data-id') )
	})
	$("div.custom-menu .delete").click(function(e){
		e.preventDefault()
		_conf("Are you sure to delete this Folder?",'delete_folder_notary',[$(this).attr('data-id')])
	})
})
	...
	$("div.file.custom-menu .delete").click(function(e){
		e.preventDefault()
		_conf("Are you sure to delete this file?",'delete_file_notary',[$(this).attr('data-id')])
	})
	$("div.file.custom-menu .download").click(function(e){
		e.preventDefault()
		window.open('download_notary.php?id='+$(this).attr('data-id'))
	})
	...
	$('.rename_file').keypress(function(e){
		...
		if(e.which == 13){
			start_load()
			$.ajax({
				url:'ajax.php?action=file_rename_notary',
				method:'POST',
				data:{id:$(this).attr('data-id'),name:$(this).val(),type:$(this).attr('data-type'),folder_id:'<?php echo $notary_parent ?>'},
...
//FILE
	...
	$('.file-item').click(function(){
    	...
    	return false;
		uni_modal($(this).attr('data-name'),'manage_files_notary.php?<?php echo $notary_parent ?>&id='+$(this).attr('data-id'))
	})
	...
	function delete_folder_notary($id){
		start_load();
		$.ajax({
			url:'ajax.php?action=delete_folder_notary',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp == 1){
	...
	function delete_file_notary($id){
		start_load();
		$.ajax({
			url:'ajax.php?action=delete_file_notary',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp == 1){
